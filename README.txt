...
README for Coursera Getting and Cleaning Data Course Project - Describes Variables

This repository contains the R script "run_analysis.R" that performs the analysis as outlined in the project requirements. This project aims to demonstrate the ability to collect, work with, and clean a data set for further analysis. The data used in this project are collected from the accelerometers of the Samsung Galaxy S smartphone.

"activity_labels" imports the different types of activities (six different types) into R.
...
"features_mean_only" is a vector of integers. It selects only the features data containing "mean". In doing this, I eliminated all fields with "meanFreq" as these are not truly means.
"features_sd_only" is a vector of integers. It selects only the features data containing "std". The rationale is the same as above.
"features_mean_or_std" is a vector of integers. It selects only the features data containing either "mean" or "std".
"set" is the working data set. Columns were combined and only means and stds were included.
"tidydata" gets the aggregate of the data by test subject ID and activity.
